...
namespace Time_Table_Management_System.Controller
{
    class addLecturerControl
    {
        ...
        public void insertLecturerDetails(addLecturerModel  add) {
            ...
            string query = "insert into lecture values('" + add.lecturerName + "','" + add.employeeId + "','" + add.faculty+ "','" + add.department + "','" + add.center + "','" + add.building + "','" + add.level + "','" + add.rank + "')";
        ...
        public  void deleteRow(int IDNumber)
        {
            try
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand("DELETE FROM lecture WHERE lid = " + IDNumber, con))
                    {
        ...
        public void updateLec(String lecName,String empID,String fclty,String dpt , String cnt,String build,int lvl,float rnk,int lecID) {
            ...
            try
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand(" UPDATE lecture SET lecturerName = '" + lecName + "', employeeId ='" + empID + "' , faculty ='" + fclty + "' , department = '" + dpt + "' , center ='" + cnt + "' , building='" + build + "' ,  level = '" + lvl + "' , rank ='" + rnk + "' WHERE   lid ='" + lecID + "'",con))
                    {
                        command.ExecuteNonQuery();
                    }
                    con.Close();
